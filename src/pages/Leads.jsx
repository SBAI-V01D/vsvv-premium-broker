...
export default function Leads() {
  ...
  const activeContractCustomerIds = useMemo(() => {
    return new Set(
      contracts.filter(c => c.status === 'active').map(c => c.customer_id).filter(Boolean)
    )
  }, [contracts])
      ...
      <div className="px-6 py-5 border-b border-border bg-card shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-[hsl(var(--primary))] tracking-tight">Lead Pipeline</h1>
              <p className="text-xs text-muted-foreground">Nur echte Akquisitions-Leads – aktive Kunden werden automatisch ausgeblendet</p>
            </div>
          </div>
