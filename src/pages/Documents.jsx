...
export default function Documents() {
  ...
  const handleReclassify = (doc, newType) => {
    updateMutation.mutate({
      id: doc.id,
      data: { doc_type: newType, classification_status: 'klassifiziert' },
    })
  }
      ...
      <div className="px-6 py-5 border-b border-border bg-card shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-[hsl(var(--primary))] tracking-tight">Dokumente ({documents.length})</h1>
              <p className="text-xs text-muted-foreground">Intelligente Klassifizierung &amp; automatische Antragserfassung</p>
            </div>
          </div>
          <div className="flex gap-2">
            {counts.pruefung_erforderlich > 0 && (
              <>
                <Button
                  onClick={() => {
                    if (confirm(`${counts.pruefung_erforderlich} Dokumente löschen?`)) {
              ...
              filtered.map((doc, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {doc.doc_type === 'antrag' && (
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground" title="KI-Extraktion" onClick={() => setReviewDoc(doc)}>
                        <Eye className="w-4 h-4" />
                      </Button>
                    )}
                    {doc.file_url && (
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground" title="Dokument öffnen" onClick={() => openDocument(doc.file_url)}>
                        <ExternalLink className="w-3.5 h-3.5" />
                      </Button>
                    )}
                    {doc.file_url && (
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground" title="Herunterladen" onClick={() => downloadDocument(doc.file_url, doc.name)}>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => { handleReclassify(doc, 'antrag'); setTimeout(() => setReviewDoc({ ...doc, doc_type: 'antrag' }), 300) }}>
                        ...
                        {doc.file_url && (
                          <DropdownMenuItem onClick={() => openDocument(doc.file_url)}>
