/**
 * TabValidation — Phase 7: Live-System-Validierung & Produktionsfreigabe
 */
  ...
  return (
    <div className="max-w-4xl space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-base font-semibold text-foreground">Phase 7 — Live-System-Validierung</h2>
          <p className="text-xs text-muted-foreground mt-0.5">
            Aktive Tests: Export-Gate · Approval-Integrität · Snapshots · Tenant-Isolation · Datenintegrität · Audit-Trail
          </p>
        </div>
        <button
          onClick={run}
      ...
      {report && (
        <>
          ...
          <div className={`border-2 rounded-xl px-5 py-4 flex items-center gap-4 ${statusCfg.bg}`}>
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                ...
                · von {report.run_by} · {new Date(report.timestamp).toLocaleString('de-CH')}
              </p>
            </div>
          </div>
                  ...
                  {tests.map((test, i) => (
                    <div key={i} className="px-5 py-3 flex items-start gap-3 text-xs">
                      ...
                      <div className="flex-1 min-w-0">
                        <span className={`font-semibold ${test.passed ? 'text-foreground' : test.severity === 'warning' ? 'text-amber-800' : 'text-red-800'}`}>
                          {test.name}
                        </span>
                        {test.details && (
          ...
          <div className="border border-border rounded-xl p-4 bg-muted/20">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Systemstatistik zum Testzeitpunkt</p>
      ...
      {!report && !loading && (
        <div className="border border-dashed border-border rounded-xl px-8 py-14 text-center">
          ...
          <p className="text-sm font-semibold text-foreground mb-1">Live-Validation noch nicht gestartet</p>
          <p className="text-xs text-muted-foreground max-w-sm mx-auto">
            Führt aktive Tests durch: Export-Gates, Approval-Integrität, Tenant-Isolation, Snapshot-Konsistenz, Datenintegrität und Audit-Trail-Vollständigkeit.
          </p>
        </div>
      )}
    </div>
