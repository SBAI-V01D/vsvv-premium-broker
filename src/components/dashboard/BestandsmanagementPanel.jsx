/**
 * BestandsmanagementPanel — Vertragsabläufe im Dashboard
 * Ganze Liste, filterbar nach Ablaufzeitraum
 */
...
import { ChevronRight, Shield, CheckCircle2, ArrowRight, AlertTriangle } from 'lucide-react'
...
function calcTopIssue(contract) {
  ...
  if (contract.status === 'expired') {
    ...
    return { type: 'ablauf', label: `Abgelaufen vor ${seit}d`, severity: 'critical', days: endDays ?? 0 }
  }

  // Kündigungsfrist
  if (cancelDays !== null && cancelDays >= -30 && cancelDays <= 90) {
    const sev = cancelDays <= 0 ? 'critical' : cancelDays <= 30 ? 'red' : cancelDays <= 60 ? 'orange' : 'yellow'
    const label = cancelDays <= 0 ? 'Kündigungsfrist abgelaufen' : `Kündigung in ${cancelDays}d`
    return { type: 'kuendigung', label, severity: sev, days: cancelDays }
  }

  // Ablaufdatum
  if (endDays !== null && endDays <= 90) {
    const sev = endDays <= 0 ? 'critical' : endDays <= 30 ? 'red' : endDays <= 60 ? 'orange' : 'yellow'
    const label = endDays <= 0 ? 'Vertrag abgelaufen' : `Läuft in ${endDays}d ab`
    return { type: 'ablauf', label, severity: sev, days: endDays }
  }
...
const SEV = {
  critical: { bg: 'bg-red-50',    border: 'border-red-200',    badge: 'bg-red-600 text-white',         dot: 'bg-red-600',    text: 'text-red-700' },
  red:      { bg: 'bg-red-50',    border: 'border-red-200',    badge: 'bg-red-100 text-red-700',       dot: 'bg-red-400',    text: 'text-red-600' },
  ...
  yellow:   { bg: 'bg-amber-50',  border: 'border-amber-200',  badge: 'bg-amber-100 text-amber-700',   dot: 'bg-amber-400',  text: 'text-amber-700' },
}
...
export default function BestandsmanagementPanel({ contracts = [] }) {
    ...
    return (
      <div className="rounded-xl border border-border bg-card overflow-hidden shadow-xs">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <span className="text-[13px] font-bold flex-1">Vertragsabläufe</span>
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden shadow-xs">
      ...
      >
        <div className={cn('w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0', criticalCount > 0 ? 'bg-red-100' : 'bg-orange-100')}>
          <Shield className={cn('w-3.5 h-3.5', criticalCount > 0 ? 'text-red-600' : 'text-orange-600')} />
        </div>
        <span className="text-[13px] font-bold flex-1">Vertragsabläufe</span>
        {criticalCount > 0 && (
          <span className="flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 bg-rose-500 text-white rounded-full">
            <AlertTriangle className="w-3 h-3" /> {criticalCount} dringend
          </span>
        )}
        {!criticalCount && allActionable.length > 0 && (
      ...
      {expanded && (
        <div className="px-4 pb-3.5">
          ...
          <div className="space-y-1.5">
            {filtered.length === 0 ? (
              <p className="py-4 text-center text-[12px] text-muted-foreground">Keine Einträge in diesem Zeitraum</p>
            ) : (
          ...
          <div className="mt-2.5">
            <button onClick={() => navigate('/vertragsablaeufe')} className="flex items-center gap-1 text-xs text-primary hover:underline font-medium">
              <ArrowRight className="w-3 h-3" /> Alle Vertragsabläufe
            </button>
          </div>
        </div>
      )}
